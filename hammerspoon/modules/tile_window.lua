...
local function layoutStackWindows(windows, boundingRect, margin, isHorizontal)
  ...
  for index, window in ipairs(windows) do
    local xOffset = isHorizontal and (index - 1) * (windowWidth + margin) or 0
    local yOffset = isHorizontal and 0 or (index - 1) * (windowHeight + margin)
...
local function setFrameWithConstraints(window, targetFrame, screenBounds)
  ...
  if hs.geometry.equals(currentFrame, targetFrame) then
    return
  end
...
local function applyLayout(screen, tilingState)
  ...
  if #managedWindows == 1 or tilingState.numberOfStackedWindows == 0 then
    ...
    local mainWindowFrame, stackBoundingRect = dividedRect(
      screenFrame,
...
local function updateManagedWindows()
  ...
  if not screen or not spaceID then
    return
  end
...
local function updateStackSize(amount)
  ...
  if not tilingState or #tilingState.managedWindows <= 1 then
    return
  end

  tilingState.numberOfStackedWindows = math.max(
    0,
...
local function promoteToMain()
  ...
  if not tilingState or #tilingState.managedWindows == 0 then
    return
  end

  for index, windowData in ipairs(tilingState.managedWindows) do
    if windowData.window:id() == window:id() then
      table.remove(tilingState.managedWindows, index)
...
local function promoteWindow()
  ...
  for index, windowData in ipairs(tilingState.managedWindows) do
    if windowData.window:id() == window:id() then
      ...
      local windowToPromote = table.remove(tilingState.managedWindows, index)

      if index > stackEnd then
        table.insert(tilingState.managedWindows, stackEnd, windowToPromote)
      else
        table.insert(tilingState.managedWindows, index - 1, windowToPromote)
      end
...
local function demoteWindow()
  ...
  for index, windowData in ipairs(tilingState.managedWindows) do
    if windowData.window:id() == window:id() then
      if index > stackEnd or index == #tilingState.managedWindows then
        return
      end

      local windowToDemote = table.remove(tilingState.managedWindows, index)

      table.insert(tilingState.managedWindows, index + 1, windowToDemote)
      tiledSpaces[spaceID] = tilingState
...
local function floatWindow()
  ...
  for index, windowData in ipairs(tilingState.managedWindows) do
    if windowData.window:id() == window:id() then
      ...
      local windowToFloat = table.remove(tilingState.managedWindows, index)
...
function module.init(config)
  if next(bindings) or windowFilter then
    module.cleanup()
  end
  ...
  local handlers = {
    ...
    increaseStackSize = function()
      updateStackSize(1)
    end,
    decreaseStackSize = function()
      updateStackSize(-1)
    end,
...
function module.cleanup()
  ...
  for spaceID, _ in pairs(tiledSpaces) do
    restoreAllWindows(spaceID)
  end
